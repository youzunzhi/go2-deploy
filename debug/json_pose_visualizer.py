...
class JsonPoseVisualizer:
    def __init__(self, json_file):
        self.json_file = json_file
        self.poses = []
        
    def load_json_data(self):
        print(f'Loading pose data from {self.json_file}...')
        ...
        for entry in data:
            ...
            orientation = entry['pose']['pose']['orientation']
        ...
        print(f'Loaded {len(self.poses)} poses from JSON file')
    
    def create_3d_visualization(self, filename='json_pose_trajectory'):
        if not self.poses:
        ...
        ax.set_ylabel('Y (m)')
        ax.set_zlabel('Z (m)')
        ax.set_title('Robot Pose Trajectory from JSON Data\n(Red=X, Green=Y, Blue=Z axes)')
        ax.legend()
        ...
        duration = self.poses[-1]['timestamp'] - self.poses[0]['timestamp']
...
def main():
    json_file = 'utlidar_robot_odom.json'
    
    visualizer = JsonPoseVisualizer(json_file)
    
    try:
        visualizer.load_json_data()
        visualizer.create_3d_visualization('json_robot_pose_trajectory')
